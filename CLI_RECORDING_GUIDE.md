...
pnpm record --classroom physics-201 --output ./output/physics.webm

# 自訂視口（1280x720）
pnpm record --classroom english-301 --output ./video.webm --viewport 1280x720

# 延長逾時（5分鐘）
pnpm record --classroom history-401 --output ./history.webm --timeout 300000

# 詳細日誌
pnpm record --classroom math-101 --output ./video.webm --verbose

# 自訂基礎 URL（開發/測試用）
pnpm record --classroom demo-001 --output ./demo.webm --base-url http://192.168.1.100:3000
```
...
| `--output` | `-o` | 輸出 WebM 檔案路徑 | `./video.webm` |
...
# 結果：./intro.webm (1920x1080, 最多120秒)
```
...
```bash
# 以手機尺寸錄製
pnpm record --classroom mobile-demo --output ./mobile.webm --viewport 375x667
```
...
```bash
# 10分鐘課程
pnpm record --classroom long-lecture --output ./long.webm --timeout 600000

# 或更簡潔的語法
pnpm record -c long-lecture -o long.webm -t 600000
```
...
**A:** 增加逾時時間
```bash
pnpm record --classroom <id> --output video.webm --timeout 300000 --verbose
```

### Q: 連接到不同的應用實例
**A:** 使用 `--base-url` 參數
```bash
pnpm record --classroom <id> --output video.webm --base-url http://other-server:3000
```

### Q: 文件很大
**A:** 使用較小的視口尺寸
```bash
pnpm record --classroom <id> --output video.webm --viewport 1280x720
```
...
**轉成 MP4（可選，需 ffmpeg）**
```bash
pnpm record --classroom math-101 --output temp.webm
ffmpeg -i temp.webm -i audio.mp3 -c:v copy -c:a aac final.mp4
```

**上傳到雲端**
```bash
pnpm record --classroom math-101 --output temp.webm && \
aws s3 cp temp.webm s3://my-bucket/classroom-recordings/
```
...
3. ✓ 輸出目錄可寫
   ```bash
   mkdir -p ./output && touch ./output/test.webm
   ```
