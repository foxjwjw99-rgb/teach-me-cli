...
function getApiHeaders(): HeadersInit {
	...
	return {
		...
		"x-video-generation-enabled": String(
			settings.videoGenerationEnabled ?? false,
		),
	};
}
...
async function generateTTSForScene(
	...
	// Pre-assign all audioIds synchronously before parallel generation
	...
	// Generate all speech actions in parallel — no ordering dependency between them
	await Promise.all(
		speechActions.map(async (action) => {
			try {
				await generateAndStoreTTS(`tts_${action.id}`, action.text, signal);
			} catch (error) {
				failedCount++;
				lastError =
					error instanceof Error
						? error.message
						: `TTS failed for action ${action.id}`;
...
export function useSceneGenerator(options: UseSceneGeneratorOptions = {}) {
	...
	const generateRemainingRef = useRef<
		((params: GenerationParams) => Promise<void>) | null
	>(null);
	...
	const generateRemaining = useCallback(
		async (params: GenerationParams) => {
			...
			const removeGeneratingOutline = (outlineId: string) => {
				...
				store
					.getState()
					.setGeneratingOutlines(current.filter((o) => o.id !== outlineId));
			};
			...
			// Generation loop with content look-ahead pipeline.
			// fetchSceneContent has no cross-scene dependency, so we start the next
			// scene's content fetch immediately after actions resolve, hiding its
			// latency behind the current scene's TTS generation.
			try {
				let pausedByFailureOrAbort = false;

				// Pre-start content fetch for the first outline
				const makeContentParams = (outline: (typeof pending)[0]) => ({
					outline,
				...
				// pending is guaranteed non-empty (early return above if empty)
				let nextContentPromise = fetchSceneContent(
					makeContentParams(pending[0]),
					signal,
				);
				...
				for (let i = 0; i < pending.length; i++) {
					...
					// Step 1: Await this scene's content (already in flight from previous iteration)
					options.onPhaseChange?.("content", outline);
					const contentResult = await nextContentPromise;

					// Immediately kick off the next scene's content fetch — runs in parallel
					// with the current scene's actions + TTS generation below.
					// For the last scene there is no next, so keep the resolved promise (it won't be awaited).
					nextContentPromise =
						i + 1 < pending.length
							? fetchSceneContent(makeContentParams(pending[i + 1]), signal)
							: nextContentPromise;
					...
					if (actionsResult.success && actionsResult.scene) {
						...
						// Extract previousSpeeches before TTS so the next scene's actions
						// are unblocked as soon as its content is ready.
						previousSpeeches = actionsResult.previousSpeeches || [];
						const settings = useSettingsStore.getState();

						// TTS generation — failure means the whole scene fails.
						// Next scene's content is already being fetched in parallel.
						if (
							settings.ttsEnabled &&
							...
							if (!ttsResult.success) {
								...
								options.onSceneFailed?.(
									outline,
									ttsResult.error || "TTS generation failed",
								);
								store.getState().setGenerationStatus("paused");
								pausedByFailureOrAbort = true;
								break;
	...
	const retrySingleOutline = useCallback(
		async (outlineId: string) => {
			...
			try {
				...
				// Step 2: Actions
				const sortedScenes = [...store.getState().scenes].sort(
					(a, b) => a.order - b.order,
				);
				const lastScene = sortedScenes[sortedScenes.length - 1];
				...
				if (
					store.getState().generatingOutlines.length > 0 &&
