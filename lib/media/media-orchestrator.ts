...
export async function generateMediaForOutlines(
	...
	// Process up to 3 requests concurrently — balances throughput vs. API rate limits
	...
	await generateSingleMedia(
		{
			...
			aspectRatio: task.params
				.aspectRatio as MediaGenerationRequest["aspectRatio"],
...
async function generateSingleMedia(
	...
	try {
		...
		const posterBlob = posterUrl
			? await fetchAsBlob(posterUrl).catch(() => undefined)
			: undefined;
		...
		const posterObjectUrl = posterBlob
			? URL.createObjectURL(posterBlob)
			: undefined;
		useMediaGenerationStore
			.getState()
			.markDone(req.elementId, objectUrl, posterObjectUrl);
	} catch (err) {
		if (abortSignal?.aborted) return;
		...
		useMediaGenerationStore
			.getState()
			.markFailed(req.elementId, message, errorCode);
