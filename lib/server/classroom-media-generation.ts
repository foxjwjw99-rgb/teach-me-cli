...
async function downloadToBuffer(url: string): Promise<Buffer> {
	...
	if (contentLength > DOWNLOAD_MAX_SIZE) {
		throw new Error(
			`File too large: ${contentLength} bytes (max ${DOWNLOAD_MAX_SIZE})`,
		);
	}
	return Buffer.from(await resp.arrayBuffer());
}

function mediaServingUrl(
	baseUrl: string,
	classroomId: string,
	subPath: string,
): string {
...
export async function generateMediaForClassroom(
	...
	const generateImages = async () => {
		await runWithConcurrency(
			imageRequests.map((req) => async () => {
				try {
					const providerId = imageProviderIds[0] as ImageProviderId;
					const apiKey = resolveImageApiKey(providerId);
					if (!apiKey) {
						log.warn(
							`No API key for image provider "${providerId}", skipping ${req.elementId}`,
						);
						return;
					}
					...
					if (result.base64) {
						...
						const urlExt = path
							.extname(new URL(result.url).pathname)
							.replace(".", "");
						ext = ["png", "jpg", "jpeg", "webp"].includes(urlExt)
							? urlExt
							: "png";
					} else {
					...
					mediaMap[req.elementId] = mediaServingUrl(
						baseUrl,
						classroomId,
						`media/${filename}`,
					);
	...
	const generateVideos = async () => {
		for (const req of videoRequests) {
			try {
				...
				if (!apiKey) {
					log.warn(
						`No API key for video provider "${providerId}", skipping ${req.elementId}`,
					);
					continue;
				}
				...
				const normalized = normalizeVideoOptions(providerId, {
					prompt: req.prompt,
					aspectRatio:
						(req.aspectRatio as "16:9" | "4:3" | "1:1" | "9:16") || "16:9",
				});
...
export function replaceMediaPlaceholders(
	...
	for (const scene of scenes) {
		...
		const canvas = (
			scene.content as {
				canvas?: {
					elements?: Array<{ id: string; src?: string; type?: string }>;
				};
			}
		)?.canvas;
...
export async function generateTTSForClassroom(
	...
	const format =
		TTS_PROVIDERS[providerId as keyof typeof TTS_PROVIDERS]
			?.supportedFormats?.[0] || "mp3";

	// Collect all speech actions across all scenes after splitting long ones
	const allSpeechItems: Array<{ action: SpeechAction }> = [];
	for (const scene of scenes) {
		...
		for (const action of scene.actions) {
			...
			allSpeechItems.push({ action: action as SpeechAction });
		}
	}

	// Generate TTS for all speech actions in parallel (concurrency 5)
	await runWithConcurrency(
		allSpeechItems.map(({ action: speechAction }) => async () => {
				...
				const result = await generateTTS(
					{
						...
						modelId:
							DEFAULT_TTS_MODELS[
								providerId as keyof typeof DEFAULT_TTS_MODELS
							] || "",
						apiKey,
						baseUrl: ttsBaseUrl,
						voice,
