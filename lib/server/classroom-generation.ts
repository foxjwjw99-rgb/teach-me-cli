...
function normalizeLanguage(language?: string): "zh-CN" | "zh-TW" | "en-US" {
...
export async function generateClassroom(
	...
	const searchQueryAiCall: AICallFn = async (
		systemPrompt,
		userPrompt,
		_images,
	) => {
	...
	if (input.enableWebSearch) {
		...
		if (tavilyKey) {
			try {
				...
				if (researchContext) {
					log.info(
						`Web search returned ${searchResult.sources.length} sources`,
					);
				}
			} catch (e) {
			...
			log.warn(
				"enableWebSearch is true but no Tavily API key configured, skipping web search",
			);
		}
	}
	...
	if (!outlinesResult.success || !outlinesResult.data) {
		...
		throw new Error(
			outlinesResult.error || "Failed to generate scene outlines",
		);
	}
		...
		...(agentMode === "generate"
			? {
					generatedAgentConfigs: agents.map((a, i) => ({
						...
						priority:
							a.role === "teacher" ? 10 : a.role === "assistant" ? 7 : 5,
					})),
				}
			: {
	...
	log.info("Stage 2: Generating scene content and actions (parallel)...");
	let generatedScenes = 0;

	// Server-side generateSceneActions has no previousSpeeches dependency, so
	// scenes are fully independent and can be generated in parallel.
	// Concurrency is limited to 3 to stay within typical LLM rate limits.
	await runWithConcurrency(
		outlines.map((outline, index) => async () => {
			const safeOutline = applyOutlineFallbacks(outline, true);
			const progressStart =
				30 + Math.floor((index / Math.max(outlines.length, 1)) * 60);
			...
			if (!content) {
				log.warn(
					`Skipping scene "${safeOutline.title}" — content generation failed`,
				);
				return;
			}
			...
			if (!sceneId) {
				log.warn(
					`Skipping scene "${safeOutline.title}" — scene creation failed`,
				);
				return;
			}
			...
			generatedScenes += 1;
			const progressEnd =
				30 + Math.floor(((index + 1) / Math.max(outlines.length, 1)) * 60);
			await options.onProgress?.({
				step: "generating_scenes",
	...
	if (input.enableImageGeneration || input.enableVideoGeneration) {
		...
		try {
			...
			log.info(
				`Media generation complete: ${Object.keys(mediaMap).length} files`,
			);
		} catch (err) {
