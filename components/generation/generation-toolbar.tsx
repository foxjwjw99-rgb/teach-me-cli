...
export interface GenerationToolbarProps {
  ...
  onLanguageChange: (lang: 'zh-CN' | 'zh-TW' | 'en-US') => void;
  webSearch: boolean;
    ...
    <div className="flex items-center gap-1 flex-wrap">
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <button
            onClick={() => {
              if (language === 'zh-TW') onLanguageChange('en-US');
              else if (language === 'en-US') onLanguageChange('zh-CN');
              else onLanguageChange('zh-TW');
            }}
            className={pillMuted}
          >
            <Globe className="size-3.5" />
            <span>{language === 'zh-TW' ? '繁中' : language === 'zh-CN' ? '簡中' : 'EN'}</span>
          </button>
        </TooltipTrigger>
